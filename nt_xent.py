...
class NTXent(torch.nn.Module):
    ...
    def forward(self, z):
        ...
        pos_idx = (sim_arg_sort == target.unsqueeze(1)).nonzero(as_tuple=True)[1]
        top1_sum = (pos_idx == 0).float().sum().item()
        top5_sum = (pos_idx < 5).float().sum().item()

        return loss, ((top1_sum, top5_sum), N * 2)
